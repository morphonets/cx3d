...
//		zAxis = normalize(zAxis);
		...
		double[][] A = Matrix.randomNoise(1.0,1000,1000);
		SystemUtilities.tic();
//		double det = det(A);
		double[][] B = Matrix.inv(A);
		SystemUtilities.tac();
	...
	public static double[] globalToLocal(double[] vectorInGlobalCoord){
		return new double[] { 
				Matrix.dot(vectorInGlobalCoord,xAxis),
				Matrix.dot(vectorInGlobalCoord,yAxis),
				Matrix.dot(vectorInGlobalCoord,zAxis)
				};
	}
