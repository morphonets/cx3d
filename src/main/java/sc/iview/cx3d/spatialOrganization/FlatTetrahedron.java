...
//		Tetrahedron<T> innerTetrahedron = getAdjacentTetrahedron(0);
		...
		int orientation = adjacentTriangles[0].orientation(point, point);
		if (orientation == 0) {
			int memory = -1;
			for (int i = 0; i < 4; i++) {
				if (adjacentTriangles[i] != null) {
					int dummy = adjacentTriangles[i].circleOrientation(point);
					if (dummy == 1)
						return 1;
					else if (dummy == 0)
						memory = 0;
	...
	public int isInConvexPosition(double[] point, int connectingTriangleNumber) {
		adjacentTriangles[0].updatePlaneEquationIfNecessary();
		if (adjacentTriangles[0].orientation(point, point) == 0)
			return 0;
		else return -1;
	}
