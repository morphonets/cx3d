...
import static sc.iview.cx3d.utilities.Matrix.printlnLine;
...
public class PhysicalSphere extends PhysicalObject{
	...
	private void updateSpatialOrganizationNodePosition() {
		...
		double offset = Matrix.norm(displacementOfTheCenter);
		if(offset>diameter*0.25 || offset > 5){

			// TODO : do we need this ?
			displacementOfTheCenter = Matrix.add(displacementOfTheCenter, Matrix.randomNoise(diameter*0.025, 3));
			try {
...
//				s.updateConcentrationBasedOnQuantity(1.0);
	 ...
	 * 
	 * @param newLength the PhysicalCylinder instance (or a class derived from it) that will be extended.
	...
	public PhysicalCylinder addNewPhysicalCylinder(double newLength, double phi, double theta){
		...
		double[] newCylinderBeginingLocation = add(massLocation, Matrix.scalarMult(radius,axisDirection));
		double[] newCylinderSpringAxis = Matrix.scalarMult(newLength,axisDirection);
		
		double[] newCylinderMassLocation = Matrix.add(newCylinderBeginingLocation, newCylinderSpringAxis);
		double[] newCylinderCentralNodeLocation = Matrix.add(newCylinderBeginingLocation, Matrix.scalarMult(0.5,newCylinderSpringAxis));
...
//		}
	...
	public boolean isInContactWithSphere(PhysicalSphere s){
		try{
			getRwLock().readLock().lock();
			double[] force = interObjectForce.forceOnASphereFromASphere(this,s);
...
//		return false;
	...
	public boolean isInContactWithCylinder(PhysicalCylinder c) {
		...
		double[] force = interObjectForce.forceOnACylinderFromASphere(c,this);
		getRwLock().readLock().unlock();
		if(Matrix.norm(force)>1E-15){
			return true;
		}else{
...
//		return f;
	...
	public void runPhysics() {	
		...
		for (int i = 0; i < daughters.size(); i++) {
			...
			rotationForce = Matrix.add(rotationForce, Matrix.crossProduct(r, forceFromDaughter));
		}
	...
	//**UNLOCK W
		...
		if(Matrix.norm(tractorForce)>0.01){
		...
		//  - make us rotate ?
		double r = Matrix.norm(rotationForce);
		...
		if(physicalTranslation || biologicalTranslation){
			...
			if(normOfTheForce*mh > Param.SIMULATION_MAXIMAL_DISPLACEMENT){
				movementAtNextStep = scalarMult(Param.SIMULATION_MAXIMAL_DISPLACEMENT, Matrix.normalize(movementAtNextStep));
			}
		...
		if(physicalRotation){
			...
			this.xAxis = Matrix.rotAroundAxis(xAxis, rotationAngle, rotationForce);
			this.yAxis = Matrix.rotAroundAxis(yAxis, rotationAngle, rotationForce);
			this.zAxis = Matrix.rotAroundAxis(zAxis, rotationAngle, rotationForce);
		}
		getRwLock().writeLock().unlock();
	...
	public double[] transformCoordinatesGlobalToPolar(double[] positionInGlobalCoordinates) {
		try
		{
			...
			double[] localCartesian = { 
					Matrix.dot(xAxis,vectorToPoint),
					Matrix.dot(yAxis,vectorToPoint),
					Matrix.dot(zAxis,vectorToPoint) };
			return new double[] {
