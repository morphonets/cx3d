...
public class PhysicalCylinder extends PhysicalObject{
	...
	private void removeProximalCylinder() {
		...
		try
		{
			...
			this.actualLength = Matrix.norm(springAxis);
	...
	void updateSpatialOrganizationNodePosition() {
		...
		if(Matrix.norm(displacementOfTheCenter)<diameter/4.0)
			return;
		...
		displacementOfTheCenter = Matrix.add(displacementOfTheCenter, Matrix.randomNoise(diameter/4.0, 3));
	...
	public void extendCylinder(double speed, double[] direction){
		getRwLock().readLock().lock();
		double temp = Matrix.dot(direction, springAxis);
		getRwLock().readLock().unlock();
		if(  temp> 0 )  {	
	...
	public void movePointMass(double speed, double[] direction){
		...
		try{
			...
			direction = Matrix.normalize(direction);
	...
	public PhysicalCylinder[] bifurcateCylinder(double length, double[] direction_1, double[] direction_2) {
		...
		if( Matrix.angleRadian(springAxis, direction_1)>Math.PI*0.5){
			double[] proj = Matrix.projectionOnto(direction_1, springAxis);
			proj = Matrix.scalarMult(-1,proj);
			direction_1 = Matrix.add(direction_1, proj);
		}
		if( Matrix.angleRadian(springAxis, direction_2)>Math.PI*0.5){
			double[] proj = Matrix.projectionOnto(direction_2, springAxis);
			proj = Matrix.scalarMult(-1,proj);
			direction_2 = Matrix.add(direction_2, proj);
		}

		// mass location and spring axis
		newBranchL.springAxis = Matrix.scalarMult(length, Matrix.normalize(direction_1));
		...
		newBranchR.springAxis = Matrix.scalarMult(length, Matrix.normalize(direction_2));
		...
		double[] newBranchCenterLocation = add(this.massLocation, Matrix.scalarMult(0.5,newBranchL.springAxis));
		SpatialOrganizationNode<PhysicalNode> newSON = null;
		try {
		...
		newBranchCenterLocation = add(this.massLocation, Matrix.scalarMult(0.5,newBranchR.springAxis));
		try {
	...
	private PhysicalCylinder extendSideCylinder(double length, double[] direction) {
		...
		if(direction == null){
			direction = Matrix.add( Matrix.perp3(springAxis), Matrix.randomNoise(0.1, 3) );
		}else{
			// check that the direction is at least 45 degrees from the branch axis
			// TODO : better method ! 
			double angleWithSideBranch = Matrix.angleRadian(springAxis, direction);
			if(angleWithSideBranch<0.78 || angleWithSideBranch > 2.35){  // 45-135 degrees
				double[] p = Matrix.crossProduct(springAxis, direction);
				p = Matrix.crossProduct(p, springAxis);
				direction = Matrix.add(Matrix.normalize(direction), Matrix.normalize(p));
			}
		}
		// location of mass and computation center
		double[] newBranchSpringAxis = Matrix.scalarMult(length, Matrix.normalize(direction));
		...
		getRwLock().writeLock().unlock();
		// new CentralNode
		double[] newBranchCenterLocation = add(this.massLocation, Matrix.scalarMult(0.5,newBranchSpringAxis));
		SpatialOrganizationNode<PhysicalNode> newSON = null;
		try {
...
//				double rresting = daughterLeft.getActualLength()+downstream.getActualLength();
				...
				double aactual = Matrix.norm(downToMe);
		
				forceOnMyPointMass = Matrix.add(forceOnMyPointMass, Matrix.scalarMult(KK*(rresting-aactual), Matrix.normalize(downToMe)));
			}
		}
...
//			double rresting = this.getActualLength()+motherCyl.getActualLength();
		...
		double normOfTheForce = Matrix.norm(forceOnMyPointMass);
		...
		double[] displacement = Matrix.scalarMult(hOverM, forceOnMyPointMass);
...
//		return (distance(P1,P2)<d1+d2);
	...
	public double[] closestPointTo(double[] p){
		try{
			...
			Matrix.printlnLine("massToP",massToP);
	...
	public void updateLocalCoordinateAxis(){
		...
		//		return;
		getRwLock().writeLock().lock();
		xAxis = Matrix.normalize(this.springAxis);
		zAxis = Matrix.crossProduct(xAxis, yAxis);
		double normOfZ = Matrix.norm(zAxis);
		if(normOfZ<1E-10){
			...
			zAxis = Matrix.perp3(xAxis);
		}else{
			zAxis = scalarMult((1/normOfZ),zAxis);
		}
		yAxis = Matrix.crossProduct(zAxis, xAxis);
		getRwLock().writeLock().unlock();
	}
	...
	public double[] transformCoordinatesGlobalToLocal(double[] positionInGlobalCoord){
		try{
			getRwLock().readLock().lock();
			positionInGlobalCoord = Matrix.subtract(positionInGlobalCoord, proximalEnd());
			return new double[] { 
					Matrix.dot(positionInGlobalCoord,xAxis),
					Matrix.dot(positionInGlobalCoord,yAxis),
					Matrix.dot(positionInGlobalCoord,zAxis)
					};
		}
		finally
		{
