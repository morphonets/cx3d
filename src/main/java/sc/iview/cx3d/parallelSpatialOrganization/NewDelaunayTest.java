...
//	public final long initialSeed = 1016542914281514000l;
	public final boolean useInitialSeed = false;
...
//	}
	...
	public static boolean checkTetrahedronForDelaunayViolation(Tetrahedron tetrahedron) throws NodeLockedException, ManagedObjectDoesNotExistException {
	...
	public void checkConvexHull() throws NodeLockedException, ManagedObjectDoesNotExistException {
...
//					}
//					else {
					// TODO: This line was commented out because SpaceNode no longer implements 
					// SpatialOrganizationNode!
...
//							SpaceNode node = anyNode;
							for (SpaceNode<PhysicalNode> node : SpaceNode.allNodes) {
								if (tetrahedron.isFlat())
									throw new RuntimeException("There is a flat tetrahedron left over!");
								if (!tetrahedron.isAdjacentTo(node) && !tetrahedron.isFlat()) try {
									if (tetrahedron.isTrulyInsideSphere(node.getPosition())) {
//									System.out.print("The Node "+node+"lies inside the circumsphere of tetrahedron "+tetrahedron+"!");
//									if (tetrahedron.orientationExact(node.getPosition()) <= 0)
//										System.out.print(" (but not truly!)");
...
//			long seed = 318763603437864272l; -8332458767730160183
			...
			SpaceNode.clear();
			Tetrahedron.clear();
			Tetrahedron<PhysicalNode> startTetrahedron;
			// TODO handle exceptions
			// TODO we need a SOM before we can do anything...
//			try {
//				startTetrahedron = Tetrahedron.createInitialTetrahedron(new SpaceNode<PhysicalNode>(
