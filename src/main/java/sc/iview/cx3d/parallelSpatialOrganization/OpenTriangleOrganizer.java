...
import static sc.iview.cx3d.utilities.Matrix.crossProduct;
import static sc.iview.cx3d.utilities.Matrix.dot;
import static sc.iview.cx3d.utilities.Matrix.normalize;
import static sc.iview.cx3d.utilities.Matrix.scalarMult;
import static sc.iview.cx3d.utilities.Matrix.subtract;
...
public class OpenTriangleOrganizer<T> {
	...
	SpatialOrganizationManager<T> som;
	 ...
	 * @param som A SpatialOrganizationManager that should host all newly created object. 
	 * Set this parameter to <code>null</code>, if each object should determine an appropriate SOM itself. 
	 */
	public OpenTriangleOrganizer(int preferredCapacity,
			AbstractTriangulationNodeOrganizer<T> tno, SpatialOrganizationManager<T> som, CacheManager<T> cm) {
		this.som = som;
		this.cm = cm;
...
//	}
	 ...
	 */
	public static <T> OpenTriangleOrganizer<T> createSimpleOpenTriangleOrganizer(CacheManager<T> cm) {//SpatialOrganizationManager<T> som) {
		return new OpenTriangleOrganizer<T>(30,	new SimpleTriangulationNodeOrganizer<T>(), null, cm);
	}
	 ...
	 * organizer, an instance of {@link SimpleTriangulationNodeOrganizer} is used 
	 * and the number of open triangles that will approximately be used is set
	 * to 30. Unlike createSimpleOpenTriangleOrganizer(), this function allows to specify a SpatialOrganizationManager
	 * that will host all newly created objects. 
	 * @param <T> The type of user objects associated with nodes in the triangulation.
	 * @param som The SpatialOrganizationManager that should host all newly created ManagedObjects.
	 * @return A new instance of <code>OpenTriangleOrganizer</code>.
	 */
	public static <T> OpenTriangleOrganizer<T> createSimpleOpenTriangleOrganizer(SpatialOrganizationManager<T> som, CacheManager<T> cm) {
		return new OpenTriangleOrganizer<T>(30,
				new SimpleTriangulationNodeOrganizer<T>(), som, cm);
	}
	 ...
	 */
	public void putTriangle(Triangle3D<T> triangle) throws NodeLockedException, ManagedObjectDoesNotExistException {
		SpaceNode<T>[] nodes = triangle.getNodes();
		TriangleHashKey<T> key = new TriangleHashKey<T>(nodes[0],
				nodes[1],
				nodes[2]);
	 ...
	 * @throws NodeLockedException 
	 */
	public void removeTriangle(Triangle3D<T> triangle) throws NodeLockedException, ManagedObjectDoesNotExistException {
		SpaceNode<T>[] nodes = triangle.getNodes();
		TriangleHashKey<T> key = new TriangleHashKey<T>(nodes[0],
				nodes[1],
				nodes[2]);
	...
	public Triangle3D<T> getTriangle(SpaceNode<T> a, SpaceNode<T> b,
		...
		if (ret == null) {
			if (som == null)
				ret = new Triangle3D<T>(a, b, c, null, null, cm);
			else
				ret = new Triangle3D<T>(a,b,c,null,null,som, cm);
			map.put(key, ret);
...
//	}
	 ...
	 * @throws NodeLockedException 
	 */
	private void removeForbiddenTriangles(LinkedList<SpaceNode<T>> sortedNodes) throws NodeLockedException, ManagedObjectDoesNotExistException {
	...
	protected void removeAllTetrahedraInSphere(
			Tetrahedron<T> startingTetrahedron) throws NodeLockedException, ManagedObjectDoesNotExistException {
	...
	private EdgeHashKey<T> triangulatePointsOnCircle(
			...
			HashMap<EdgeHashKey<T>, EdgeHashKey<T>> map,
			LinkedList<Triangle3D<T>> triangleList) throws NodeLockedException, ManagedObjectDoesNotExistException {
		if (startingEdge != null) {
	...
	private void triangulatePointsOnSphere(
			...
			Triangle3D<T> startingTriangle) throws NodeLockedException, ManagedObjectDoesNotExistException {
		LinkedList<Triangle3D<T>> surfaceTriangles = new LinkedList<Triangle3D<T>>();
		SpaceNode<T>[] startingTriangleNodes = startingTriangle.getNodes();
	...
	private void createInitialTriangle(
			AbstractTriangulationNodeOrganizer.DistanceReporter rep) throws NodeLockedException, ManagedObjectDoesNotExistException {
		...
		if (som == null)
			putTriangle(new Triangle3D<T>(a, b, c, null, null, cm));
		else
			putTriangle(new Triangle3D<T>(a, b, c, null, null, som, cm));
	}
	...
	private void createNewTetrahedron(
			Triangle3D<T> openTriangle, SpaceNode<T> oppositeNode) throws NodeLockedException, ManagedObjectDoesNotExistException {
		if (som == null)
			aNewTetrahedron = new Tetrahedron<T>(openTriangle,	oppositeNode, this, cm);
		else
			aNewTetrahedron = new Tetrahedron<T>(openTriangle,	oppositeNode, this, som, cm);
	 ...
	 * triangle. The triangle node organizer (given as argument to OpenTriangleOrganizer(int, AbstractTriangulationNodeOrganizer))
	 ...
	 *    
	 */
	public void triangulate() throws NodeLockedException, ManagedObjectDoesNotExistException {
